...
export class InvalidPathMultiOrderError extends Error {
	...
	constructor(path: SemiMarket[]) {
		super(InvalidPathMultiOrderError.buildMessage(path))
		this.path = path
	}

	private static buildMessage(path: SemiMarket[]): string {
		const firstMarket = path.at(0)
		if (!firstMarket) {
			return 'Invalid path for multi order, path is empty.'
		}
		if (path.length === 1) {
			return 'Invalid path for multi order, path is a single market'
		}
		...
		return `Invalid path for multi order (incorrect links between markets), got: ${pathString}.`
	}
}
